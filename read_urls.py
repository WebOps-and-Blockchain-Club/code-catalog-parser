...
def read_code_from_output():
    with open(f"gitHubSources/{dir}/documentation/{repo}.json", "r") as f:
        data = json.load(f)
    ...
    for item in data:
        ...
        called_function_ids = [
            id
            for id, func in function_ids.items()
            if func in called_functions and id != item["id"]
        ]
        item["functions_called"] = called_function_ids

    with open(f"gitHubSources/{dir}/documentation/{repo}.json", "w") as f:
...
def get_final_output_json():
    ...
    write_output_to_json(output_data, f"gitHubSources/{dir}/documentation/{repo}.json")
    # print(output_data)
